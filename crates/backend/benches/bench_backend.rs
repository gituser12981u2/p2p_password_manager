...
fn create_test_header() -> PinsetHeader {
    ...
    PinsetHeader::builder(1, AeadAlgorithm::AesGcm, KeySource::OsKeyStore, store_id, nonce)
        .seq(42)
        .kdf("argon2id")
        .build()
        .unwrap()
...
fn bench_header_encode(c: &mut Criterion) {
    ...
    c.bench_function("header_encode", |b| {
        b.iter(|| {
            let encoded = black_box(&header).encode().unwrap();
            black_box(encoded);
        })
    });
}
...
fn bench_header_decode(c: &mut Criterion) {
    ...
    c.bench_function("header_decode", |b| {
        b.iter(|| {
            let decoded = PinsetHeader::decode(black_box(&encoded)).unwrap();
            black_box(decoded);
        })
    });
}
...
fn bench_record_encode(c: &mut Criterion) {
    ...
    c.bench_function("record_encode", |b| {
        b.iter(|| {
            let encoded = black_box(&record).encode().unwrap();
            black_box(encoded);
        })
    });
}
...
fn bench_record_decode(c: &mut Criterion) {
    ...
    c.bench_function("record_decode", |b| {
        b.iter(|| {
            let decoded = black_box(PinsetRecord::decode(black_box(&encoded)).unwrap());
            black_box(decoded);
        })
    });
}
...
fn bench_record_roundtrip(c: &mut Criterion) {
    ...
    c.bench_function("record_roundtrip", |b| {
        b.iter(|| {
            let encoded = black_box(&record).encode().unwrap();
            let decoded = PinsetRecord::decode(black_box(&encoded)).unwrap();
            black_box(decoded);
        })
    });
}
...
fn bench_record_encode_varying_key_sizes(c: &mut Criterion) {
    ...
    for key_size in [32, 64, 128, 256, 512, 1024].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(key_size), key_size, |b, &size| {
            let mut rng = rand::thread_rng();
            let peer_id: [u8; 32] = rng.r#gen();
        ...
        group.bench_with_input(BenchmarkId::from_parameter(count), count, |b, &n| {
            ...
            b.iter(|| {
                let encoded: Vec<_> = records
                    .iter()
                    .map(|r| black_box(r.encode().unwrap()))
                    .collect();
                black_box(encoded);
            });
        });
...
fn bench_multiple_records_decode(c: &mut Criterion) {
    ...
    for count in [10, 50, 100, 500, 1000].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(count), count, |b, &n| {
            let records: Vec<_> = (0..n).map(|_| create_test_record()).collect();
            let encoded: Vec<_> = records.iter().map(|r| r.encode().unwrap()).collect();

            b.iter(|| {
                let decoded: Vec<_> = encoded
                    .iter()
                    .map(|e| PinsetRecord::decode(black_box(e)).unwrap())
                    .collect();
                black_box(decoded);
            });
        });
